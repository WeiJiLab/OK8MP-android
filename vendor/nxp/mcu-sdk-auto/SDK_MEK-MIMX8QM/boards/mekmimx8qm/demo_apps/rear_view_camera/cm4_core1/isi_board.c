...
#define assert_scfw(err, str) \
    if (SC_ERR_NONE != err) \
    { \
        PRINTF("err code:%d--"str, err); \
        return; \
    }
...
{
    sc_rm_pt_t pt_boot = AP_BOOT_PARTITION_ID;
    sc_ipc_t ipc;
    sc_err_t err = SC_ERR_NONE;

    ipc = SystemGetScfwIpcHandle();

    err = sc_pm_boot(ipc, pt_boot, AP_BOOT_CPU, AP_BOOT_ADDR, AP_BOOT_MU, AP_BOOT_DEV);
    assert_scfw(err, "SC ERR:boot AP Core failed\r\n");
}

void SOC_AssignDisplayCamera(sc_rm_pt_t pt)
{
    sc_rm_pt_t pt_android = pt;
    sc_rm_pt_t pt_m4_1;
    sc_ipc_t ipc;
    sc_err_t err = SC_ERR_NONE;

    ipc = SystemGetScfwIpcHandle();
    err = sc_rm_get_partition(ipc, &pt_m4_1);
    assert_scfw(err, "SC ERR:sc_rm_get_partition failed\r\n");

    sc_rm_set_resource_movable(ipc, SC_R_ALL, SC_R_ALL, SC_FALSE);
    sc_rm_set_pad_movable(ipc, SC_P_ALL, SC_P_ALL, SC_FALSE);

    sc_rm_set_subsys_rsrc_movable(ipc, MIPI_CSI_RSRC, SC_TRUE);
//    sc_rm_set_subsys_rsrc_movable(ipc, SC_R_CSI_1, SC_TRUE);
    sc_rm_set_subsys_rsrc_movable(ipc, LDB_RSRC, SC_TRUE);
//    sc_rm_set_subsys_rsrc_movable(ipc, SC_R_LVDS_0, SC_TRUE);
    sc_rm_set_subsys_rsrc_movable(ipc, DC_RSRC, SC_TRUE);
//    sc_rm_set_subsys_rsrc_movable(ipc, SC_R_DC_0, SC_TRUE);
    sc_rm_set_resource_movable(ipc, ISI_RSRC, ISI_RSRC, SC_TRUE);

    sc_rm_set_pad_movable(ipc, SC_P_M41_I2C0_SCL, SC_P_M41_GPIO0_01, SC_TRUE);
//    sc_rm_set_pad_movable(ipc, SC_P_LVDS0_I2C0_SDA, SC_P_LVDS0_I2C1_SDA, SC_TRUE);
    sc_rm_set_pad_movable(ipc, SC_P_LVDS0_I2C0_SCL, SC_P_LVDS0_I2C0_SCL, SC_TRUE);
    sc_rm_set_pad_movable(ipc, SC_P_LVDS1_I2C1_SCL, SC_P_LVDS1_I2C1_SDA, SC_TRUE);

    sc_rm_set_pad_movable(ipc, SC_P_MIPI_CSI0_MCLK_OUT, SC_P_MIPI_CSI0_GPIO0_01, SC_TRUE);
//    sc_rm_set_pad_movable(ipc, SC_P_MIPI_CSI1_MCLK_OUT, SC_P_MIPI_CSI1_I2C0_SDA, SC_TRUE);
//    sc_rm_set_pad_movable(ipc, SC_P_MIPI_DSI0_I2C0_SCL, SC_P_MIPI_DSI0_I2C0_SDA, SC_TRUE);
//    sc_rm_set_pad_movable(ipc, SC_P_MIPI_DSI1_I2C0_SCL, SC_P_MIPI_DSI1_I2C0_SDA, SC_TRUE);

    err = sc_rm_move_all(ipc, pt_m4_1, pt_android, SC_TRUE, SC_TRUE);
    assert_scfw(err, "SC ERR:sc_rm_move_all failed\r\n");

#if 0
    err = sc_rm_set_master_sid(ipc, SC_R_M4_1_PID0, 0x9);
    assert_scfw(err, "SC ERR:set sid failed\r\n");
    PRINTF("err code:%d--", err);
#endif

    unsigned int resource_array[13][2] = {
           {SC_R_DC_1_BLIT0, 0x13},
           {SC_R_DC_1_BLIT1, 0x13},
           {SC_R_DC_1_BLIT2, 0x13},
           {SC_R_DC_1_BLIT_OUT, 0x13},
           {SC_R_DC_1_WARP, 0x13},
           {SC_R_DC_1_VIDEO0, 0x13},
           {SC_R_DC_1_VIDEO1, 0x13},
           {SC_R_DC_1_FRAC0, 0x13},
           {SC_R_DC_1, 0x13},
           {SC_R_ISI_CH0, 0x5},
           {SC_R_ISI_CH1, 0x5},
           {SC_R_ISI_CH2, 0x5},
    ...
    for (i = 0; i < 13; i++)
    {
       err = sc_rm_set_master_sid(ipc, resource_array[i][0], resource_array[i][1]);
       assert_scfw(err, "SC ERR: sc_rm_set_master_sid failed\r\n");
    }

    return;
}

#define APP_MU_SCU_IRQ_PRIORITY 3
//#define SCU_MU_CHANNEL          0
#define APP_M4_SCU_MU LSIO__MU12_B
#define APP_M4_SCU_MU_IRQn LSIO_MU12_INT_B_IRQn
...
int platform_scu_mu_init(void)
{
     ...
     */
    MU_Init(APP_M4_SCU_MU);
    NVIC_SetPriority(GET_IRQSTEER_MASTER_IRQn(APP_M4_SCU_MU_IRQn), APP_MU_SCU_IRQ_PRIORITY);
    NVIC_EnableIRQ(GET_IRQSTEER_MASTER_IRQn(APP_M4_SCU_MU_IRQn));
    IRQSTEER_EnableInterrupt(IRQSTEER, APP_M4_SCU_MU_IRQn);

    MU_EnableInterrupts(APP_M4_SCU_MU, kMU_GenInt3InterruptEnable | kMU_Rx3FullInterruptEnable);
    return 0;
}

int LSIO_MU12_INT_B_IRQHandler(void)
{
//    uint32_t channel;
    vehicle_state_t stateMsg;
    BaseType_t xHigherPriorityTaskWoken;

    MU_ClearStatusFlags(APP_M4_SCU_MU, kMU_GenInt3Flag | kMU_Rx3FullFlag);

    // xHigherPriorityTaskWoken must be initialised to pdFALSE.
    xHigherPriorityTaskWoken = pdFALSE;
    stateMsg.type = STATE_TYPE_AP_POWER;
    stateMsg.value = 0;/* TODO: set actual AP power state*/

//    if (((1U << 27U) >> SCU_MU_CHANNEL) & MU_GetStatusFlags(APP_M4_SCU_MU))
//    {
//        channel = MU_ReceiveMsgNonBlocking(APP_M4_SCU_MU, SCU_MU_CHANNEL); /* Read message from RX register. */
//    }

    //xEventGroupSetBitsFromISR(g_xAPStatus, AP_STATUS_REBOOT, &xHigherPriorityTaskWoken);
    xQueueSendFromISR( xVStateQueue, &stateMsg, &xHigherPriorityTaskWoken );
