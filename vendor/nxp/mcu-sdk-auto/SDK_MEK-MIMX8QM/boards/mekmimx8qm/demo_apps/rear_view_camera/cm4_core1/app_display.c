...
/*
  The DPU path is configured like this:
       ...
       +-----------------+     +-----------------+       +-----------------+
       |                 |     |                 |       |                 |
       |   FetchDecode0  |     |  FetchLayer0    |       |  Const Frame 4  |
       |                 |     |                 |       |                 |
       +-----------------+     +-----------------+       +-----------------+
               |                        |                         |
               |                        v                         |
 ...
 */
void APP_InitDisplay(uint32_t frameBuffer, uint32_t overlayBuffer, app_display_callback_t callback)
{
    ...
    DPU_InitLayerBlend(APP_DPU, kDPU_LayerBlend0,
                       DPU_MAKE_SRC_REG2(APP_LAYERBLEND_SECOND_SRC, kDPU_UnitSrcFetchLayer0));
    DPU_LayerBlendGetDefaultConfig(&lbConfig);
    lbConfig.secAlphaBlendMode  = kDPU_BlendSecAlpha;
    lbConfig.primAlphaBlendMode = kDPU_BlendOne;
    lbConfig.secColorBlendMode  = kDPU_BlendOne;
    lbConfig.primColorBlendMode = kDPU_BlendOne;
    DPU_SetLayerBlendConfig(APP_DPU, kDPU_LayerBlend0, &lbConfig);
    DPU_EnableLayerBlend(APP_DPU, kDPU_LayerBlend0, true);

    /*
     * Fetch unit for Fetch Layer.
     */
    ...
    /* Sublayer 0. */
    sbConfig.strideBytes  = 4 * APP_FRAME_WIDTH;
    sbConfig.bufferHeight = APP_FRAME_HEIGHT;
    sbConfig.bufferWidth  = APP_FRAME_WIDTH;
    sbConfig.baseAddr     = overlayBuffer;
    ...
    s_displayHandle.activeFb       = 0; /* No active frame buffer */
...
void APP_DeinitDisplay(void)
{
    DPU_StopDisplay(APP_DPU, APP_DPU_DISPLAY_INDEX);
    APP_StopDisplayInterface();
    DPU_Deinit(APP_DPU);
    APP_UnprepareDisplay();
}
