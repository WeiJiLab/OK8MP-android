 ...
 * Definitions
 ******************************************************************************/
#define I2C_COMMON_RETRY    5

#define assert_i2c_status(func, err_log) \
do { \
    int retry; \
    for (retry=0; retry<I2C_COMMON_RETRY; retry++) { \
        status = func; \
    ...
    if (retry == I2C_COMMON_RETRY) \
        PRINTF("I2C failed after retry %d times:"err_log"\r\n", I2C_COMMON_RETRY); \
}while(0)
...
static status_t OV10635_Init(camera_device_handle_t *handle, uint8_t index)
{
    ...
    {
        PRINTF("Failed: OV10635_Read OV10635_REG_PID!\r\n");
        return kStatus_Fail;
    }
    ...
    {
        PRINTF("Failed: OV10635_Read OV10635_REG_VER!\r\n");
        return kStatus_Fail;
    }
    ...
    for (i = 0; i < ARRAY_SIZE(ov10635Firmware); i++)
    {
        ...
        if (status != kStatus_Success)
        {
            PRINTF("Failed: OV10635_Write ov10635Firmware!\r\n");
            return status;
        }
    }
                                     ...
                                     const camera_config_t *config)
{
    ...
    {
        PRINTF("Failed: resolutionConfig NULL!\r\n");
    ...
    /* Enable PRBS test */
    MAX9286_Write(handle, 0x0E, 0x5F);
    VIDEO_DelayMs(10);

    /* Enable Custom Reverse Channel & First Pulse Length*/
    assert_i2c_status(MAX9286_Write(handle, 0x3F, 0x4F), "Enable Custom Reverse Channel & First Pulse Length");
    VIDEO_DelayMs(2);

    /* First pulse length rise time changed from 300ns to 200ns */
    assert_i2c_status(MAX9286_Write(handle, 0x3B, 0x1E), "First pulse length rise time changed from 300ns to 200ns");
    VIDEO_DelayMs(2);
    ...
    assert_i2c_status(MAX9286_Write(handle, 0x3B, 0x19), "Increase reverse amplitude from 100mV to 170mV");
    VIDEO_DelayMs(5);
     ...
     */
    assert_i2c_status(MAX9286_Write(handle, 0x12, 0xF3), "CSI-2 Output uses YUV 422 8-bit");

    /* Internal frame sync uses automatic mode */
    assert_i2c_status(MAX9286_Write(handle, 0x01, 0x02), "Internal frame sync uses automatic mode");
     ...
     */
    assert_i2c_status(MAX9286_Write(handle, 0x00, 0xEF), "Auto detect link used for CSI clock source");
     ...
     */
    assert_i2c_status(MAX9286_Read(handle, 0x49, &enabledLink), "Detect available link");
    ...
    {
        /* No valid link. */
        PRINTF("Failed: enabledLink==0, The camera not valid.!\r\n");
        return kStatus_Fail;
    }

    /* Disable PRBS test */
    MAX9286_Write(handle, 0x0E, 0x50);

    /* Select the first valid. */
    enabledLink = (~(enabledLink - 1)) & enabledLink;

    assert_i2c_status(MAX9286_ReorderVC(handle, enabledLink), "Select the first valid");

    /* Enable available links. */
    assert_i2c_status(MAX9286_Write(handle, 0x00, 0xE0 | enabledLink), "Enable available links");
    ...
    for (i = 1; i <= MAX9271_MAX_COUNT; i++)
    {
        ...
        assert_i2c_status(MAX9286_Write(handle, 0x0A, (0x11 << (i - 1))), "Enable Link i Reverse Channel");
    ...
    assert_i2c_status(MAX9286_Write(handle, 0x0A, enabledLink | (enabledLink << 4U)), "Enable Link Reverse Channel");

    /* Disable auto acknowledge. */
    assert_i2c_status(MAX9286_Write(handle, 0x34, 0x36), "Disable auto acknowledge");
    ...
    for (i = 1; i <= MAX9271_MAX_COUNT; i++)
    {
        ...
        if (status != kStatus_Success)
        {
            PRINTF("OV10635_SetResolutionConfig Failed!\r\n");
            return status;
        }
    }

    /* Enable auto acknowledge. */
    assert_i2c_status(MAX9286_Write(handle, 0x34, 0xB6), "Enable auto acknowledge");
    ...
    MAX9286_Write(handle, 0x15, 0x9B);
    VIDEO_DelayMs(10);
    return kStatus_Success;
}
...
status_t MAX9286_InitExt(camera_device_handle_t *handle, const camera_config_t *config, const void *extConfig)
{
    ...
    {
        PRINTF("MAX9286_Read MAX9286_REG_ID failed!\r\n");
        return kStatus_Fail;
    }
