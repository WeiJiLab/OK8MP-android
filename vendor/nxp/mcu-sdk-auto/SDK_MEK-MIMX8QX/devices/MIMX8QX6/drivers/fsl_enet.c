...
static const IRQn_Type s_enetTsIrqId[] = ENET_Transmit_IRQS;
...
void ENET_ReleaseRxBuffer(ENET_Type *base, enet_handle_t *handle, void *buffer, uint8_t ringId)
{
    ...
    enet_rx_bd_ring_t *rxBdRing                    = &handle->rxBdRing[ringId];
    volatile enet_rx_bd_struct_t *ownBuffDescrip   = rxBdRing->rxBdBase;
    volatile enet_rx_bd_struct_t *blockBuffDescrip = rxBdRing->rxBdBase + rxBdRing->rxGenIdx;
    volatile enet_rx_bd_struct_t tempBuffDescrip;
    uint16_t index   = rxBdRing->rxGenIdx;
    ...
    if (isReleaseBd)
    {
        ...
        do
        {
            ...
            {
                blockBuffDescrip = rxBdRing->rxBdBase;
            }
            else
            {
                blockBuffDescrip++;
