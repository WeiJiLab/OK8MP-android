...
{
#ifdef CONFIG_FORLINX_LOGO_KEEP
	if(lcdifv3->logo < 4 && !lcdifv3->is_hdmi ) {
		lcdifv3->logo ++;
		return;
	}
#endif
...
#ifdef CONFIG_FORLINX_LOGO_KEEP
bool lcdifv3_is_hdmi(struct lcdifv3_soc *lcdifv3)
{
	return lcdifv3->is_hdmi;
}

EXPORT_SYMBOL(lcdifv3_is_hdmi);
#endif
...
#ifdef CONFIG_FORLINX_LOGO_KEEP
	lcdifv3->is_hdmi = soc_pdata->hdmimix;
#endif
