...
#ifdef CONFIG_FORLINX_LOGO_KEEP
#define PHY_REF_CLK     24000
#else
				...
				const struct drm_display_mode *mode)
{
	...
	if (bpp < 0)
		return -EINVAL;
#ifdef CONFIG_FORLINX_LOGO_KEEP
	pix_clk = DIV_ROUND_UP(mode->clock * 10, 9);
#else
	pix_clk = mode->clock;
#endif
...
extern int call_imx8mp_pm_notifiers(unsigned long event);
#endif
...
#ifdef  CONFIG_FORLINX_LOGO_KEEP
	uint32_t dsi_status;
	dsi_status = dsim_read(dsim, DSIM_STATUS);
	if(! (dsi_status & STATUS_PLLSTABLE)){
		       ...
		       int irq, const struct sec_mipi_dsim_plat_data *pdata)
{
	...
	clk_prepare_enable(dsim->clk_pllref);
	clk_prepare_enable(dsim->clk_cfg);
	sec_mipi_dsim_irq_mask(dsim, PLLSTABLE);
#endif
